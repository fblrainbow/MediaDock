...
界面显示文件大小：任务行和清晰度下拉框都能看到预计/实际大小。

**新增**

- `/formats` 的 `presets[]` 新增 `size_bytes`：按预设估算（视频预设 = 封顶内最佳视频
  + 最佳音频；`audio` = 最佳音频），无法估算时返回 `0`（组件缺失时**绝不**用部分大小冒充）。
- `/tasks` 与 `/history` 的任务项新增 `size_bytes`：下载中 = yt-dlp 报出的总大小；
  已完成 = 磁盘上真实文件大小；其他状态 = `0`（前端不显示）。
- 用户脚本：任务行显示大小
  （`⏳ 标题 · 42.3% · 368.6 MB · 1.5MiB/s · ETA 01:30`、`✅ 标题 · 完成 · 10.0 MB`）；
  清晰度下拉显示估算大小（`1080p · ≈245 MB`）。
- 打开视频页时先查一次 `/formats` 填充下拉框（同 URL 只查一次，服务端有缓存，
  下载时直接复用，不会多跑一次 yt-dlp）。

**不变**

- Task 字段、状态集合、存储 schema、API 错误码均未变化（`size_bytes` 是增量字段）。
- yt-dlp argv、默认 1080p MP4 策略、「仅音频 = MP3」全部不变。

**验收证据**：378 个单元测试、9 个探针（`probe_formats` 45 项）、发布检查 60 项全部通过。
...
| --- | --- | --- | --- |
| `1.0.3` | `5.2` | `2` | Stage-013（界面显示文件大小） |
