...
- [ ] MVP 主流程可重复执行。
- [ ] Task 模型、状态和 API 契约有单一来源。
- [ ] 每个已完成阶段都有阶段验收和影响检查记录。
- [ ] 受影响的前置阶段已经完成回归测试。
- [ ] 后续阶段计划已经根据实际输出同步更新。
- [ ] 正常、边界、异常、兼容性和安全测试有结果记录。
- [ ] 未完成任务按百分比降序，完成任务按完成时间倒序。
- [ ] 默认最多显示 20 个任务，未完成任务超量时可滚动查看。
- [ ] 完成任务过多时默认折叠，并可以展开查看完成顺序。
- [ ] 所有打开的 YouTube 页面看到一致的任务列表和状态。
- [ ] 配置、依赖、日志、数据、迁移和回滚有明确说明。
- [ ] 发布后验证和问题升级流程可执行。
- [ ] 已知限制和未完成能力没有被写成已支持能力。

---

## 17. 当前第一步

下一步不是立即开发多平台或复杂 UI，而是生成并执行 `Stage-001.md`：

1. 记录当前代码和 API 基线。
2. 补齐最小测试结构。
3. 验证 YouTube 单任务端到端流程。
4. 确认 yt-dlp、FFmpeg、路径、日志和错误边界。
5. 完成 Stage-001 的前后影响检查。
6. 只有验收通过后，才生成或执行 `Stage-002.md`。
