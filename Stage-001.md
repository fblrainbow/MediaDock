...
- 开始时间：2026-09-19
- 预计完成时间：2026-09-19
- 当前阻塞：无；仅 T015（SPA 导航）未单独验证，遗留到 Stage-003
- 前置阶段：无
...
- [x] `/health` 返回 HTTP 200 和 JSON `status=ok`。
- [x] 合法 URL 能创建任务并返回唯一 `task_id`。
- [x] `/status` 能查询全量任务。
- [x] `/status?id=...` 能查询指定任务。
- [x] 页面能够显示下载中的百分比、速度和 ETA。
- [x] 下载成功时能生成最终 MP4 并显示 `completed`。
- [x] 下载失败时显示 `error`，并能定位错误原因。

### 8.2 兼容性验收

- [x] 保持 `127.0.0.1:8765`。
- [x] 保持当前 `GET /download?url=...`。
- [x] 保持 `/health`、`/status` 的当前可用行为，除非记录了兼容变更。
- [x] 不改变 MVP 默认格式选择和 MP4 合并策略。
- [x] Windows 环境可启动服务并写入下载目录。

### 8.3 质量与安全验收

- [x] `python -m py_compile .\server.py` 通过。
- [x] URL 协议校验测试通过。
- [x] 不默认监听 `0.0.0.0`。
- [x] 依赖缺失时不会伪造成功状态。
- [x] 已记录日志、路径和完整 URL 可能带来的安全风险。
- [x] 已知限制没有写成已实现能力。
...
- [x] 本阶段无前置阶段。
- [x] 已确认当前代码实际行为与 `plan.md` 和 `plan-whole.md` 的 MVP 目标差异。

**对后续阶段：**

- [x] 已向 Stage-002 输出 Task 字段、状态、API 和错误行为快照。
- [x] 已明确 Stage-002 不能继续依赖单一 `currentTaskId` 作为全局任务模型。
- [x] 已明确 Stage-003 需要重做任务列表和跨页面同步。
- [x] 已记录所有需要回归的 MVP API。
- [x] 如果本阶段发现 API、Task 或格式策略需要改变，已先更新 `plan-whole.md` 和本文件版本。

**进入下一阶段条件：**

- [x] 所有高影响决策已确认，或已明确记录保守假设。
- [x] 核心测试结果已记录。
- [x] 阻塞项已有处理决定。
- [x] 允许生成 `Stage-002.md`。
...
- 实际输出：`docs/stage001-baseline.md` 快照；`tests/` 15 项通过 + 假 yt-dlp 失败链路探针；`requirements.txt` 明确 stdlib-only；`server.py` 零改动。
- 差异：T011/T012 真实下载未执行，T014/T015 手工未执行；API 验证用临时端口而非占用 8765。
- 差异影响：成功路径 MP4 合并尚未实证；后续阶段不得把成功下载写成已验证。
- 处理决定：接受差异，阻塞项留待后续真实验证；不回溯计划版本。
...
| --- | --- | --- | --- | --- | --- | --- |
| 2026-09-19 | 任务 001/007 | Cline | 完成 | 记录 API/Task/yt-dlp/日志/前端基线到 `docs/stage001-baseline.md` | 通过（读码） | 无 |
| 2026-09-19 | 任务 002 | Cline | 完成 | 确认 venv Python 3.13.15、yt-dlp 2026.08.19、ffmpeg 9.0.1、downloads 目录、127.0.0.1 绑定 | 通过 | 未做依赖缺失破坏测试 |
| 2026-09-19 | 任务 003 | Cline | 完成 | 新增 `tests/test_baseline.py`、`tests/test_api.py`（stdlib）；15/15 通过，见 `tests/unittest_result.txt`；`requirements.txt` 记为 stdlib-only | 通过 | 无 |
| 2026-09-19 | 任务 004 | Cline | 完成 | 健康/全量/单任务/缺参/非法 URL/不存在任务 API 验证通过；合法创建任务经假 yt-dlp 探针验证 | 通过 | 用临时端口探针，未占用 8765 真实端口 |
| 2026-09-19 | 任务 005 | Cline | 阻塞 | 真实 YouTube 下载未执行；仅用假 yt-dlp 验证失败路径 error 可观测 | 未执行 | T011/T012 留给后续真实网络验证 |
| 2026-09-19 | 任务 006 | Cline | 阻塞 | 仅静态记录 MediaDock.js 行为，未做浏览器手工验证 | 未执行 | T014/T015 留待手工验证 |
| 2026-09-19 | 基线整理 | Cline | 完成 | 输出快照与测试证据；`server.py` 零改动 | 通过 | 无 |
| 2026-09-19 | 任务 005/006 | 用户 | 完成 | 真实 YouTube 端到端下载验证通过，符合预期（按钮创建任务并显示状态，最终完成） | 通过 | T011/T012/T014/T015 关闭 |
| 2026-09-19 | 服务中断排查 | 用户+Cline | 完成 | 发现 8765 无监听（连接拒绝），按规范用 venv 重启为单进程 PID 2192，`/health` 200；页面恢复可下载 | 通过 | 后续验证前先确认 8765 有监听 |
| 2026-09-19 | T013/T016 补记 | Cline | 完成 | T013 以假 yt-dlp 探针为据通过（returncode=1 → status=error + 日志 `failed: returncode=1`）；T016 以 venv 重启后 `/status` 任务数 0 + 重启后用户真实下载成功为据通过 | 通过 | 仅 T015（SPA 导航）未单独验证，遗留 Stage-003 |
| 2026-09-19 | 阶段签字 | 用户 | 完成 | 确认验收 8.1–8.4 全部通过，允许进入 Stage-002 | 通过 | 无 |
...
- 阶段状态：已完成
- 阶段验收结论：允许进入 Stage-002
- 对前置阶段影响：无（本阶段无前置阶段）
- 对后续阶段影响：Stage-002 获得 Task/API/错误快照（`docs/stage001-baseline.md`）；Stage-003 需重做任务列表与跨页面同步，并补 T015（SPA 导航）验证
- 是否更新 `plan-whole.md`：是，阶段总览增加完成时间列，Stage-001 填 2026-09-19
- 审查人：用户
- 日期：2026-09-19
